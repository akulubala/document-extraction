...
class Extraction:
    ...
    def load_from_pdf(self):
        ...
        try:
            pdf_reader = SimpleDirectoryReader(
                input_files=[Path(f"/srv/extract-files/{self.document_name}")])
    ...
    def load_from_excel(self):
        try:
            extracted_data = process_excel(Path(f"/srv/extract-files/{self.document_name}"))
    ...
    def load_from_image(self) -> list:
        ...
        for image_path in self.images:
            base64_image = encode_image(Path(f"/srv/extract-files/{image_path}"))
